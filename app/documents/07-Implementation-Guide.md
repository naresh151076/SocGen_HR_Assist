...
| Connected example conversation | Implemented as a local, continuous role-aware chat across Claire, Amélie, Radu, and Elena |
...
| `app/components/Workbench.tsx` | Time-aware landing greeting, registration-updates chart, continuous example conversation, Claire's Team learning summary, Help guidance, and fixed composer |
...
The example request is displayed as an application-like conversation rather than a numbered workflow: it starts with a credible prefilled user request, then alternates user messages, assistant outcomes, focused questions, and compact decision controls. It uses fictional local data and never claims live actions.
